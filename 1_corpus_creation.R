...
corpus_udp <- udpipe(x = corpus$text, object = "spanish") %>% 
  filter(upos != "PUNCT") # remove punctuation from tokens
...
save(corpus_lines, file = "corpus_objects/corpus_lines.rda")
save(corpus_token, file = "corpus_objects/corpus_token.rda")
